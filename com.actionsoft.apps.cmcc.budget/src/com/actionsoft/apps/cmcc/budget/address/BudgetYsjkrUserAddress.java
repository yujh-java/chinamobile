...
/**
 * 预算管理 财务部审核节点，转办时，找预算接口人
 * 
 * @author wxx
 *
 */
public class BudgetYsjkrUserAddress implements AddressUIFilterInterface {
	// 在流程数据维护表中获取需求部门接口人部门ID
	String sql_deptId = "select DEMAND from BO_ACT_CMCC_PROCESSDATA where PROCESSID = ?";
	/*
	 * 在企标角色维护表中：BO_ACT_ENTERPRISE_ROLEDATA 查询需求部门企标管理员
	 */
	...
	public String roleName_sql = "select count(*) c " + "from BO_ACT_ENTERPRISE_ROLEDATA " + "where rolename = ? "
			+ "and userid = ?";
	// 用于判断是否企标管理员
	public String count;
	...
	/*
	 * private String leaderDeptId = "";//处长所在部门ID private List<String>
	 * list_userPathids = new ArrayList<String>();//部长所在部门全路径 private
	 * List<String> list_userids = new ArrayList<String>();//部长角色所有人员账号
	 */ @Override
	public boolean addressUIFlexDepartmentFilter(UserContext uc, DepartmentModel model,
		...
		/*
		 * if("".equals(deptId)){ return false; }
		 */
		// 技术部门ID全路径即为技术部门ID全路径
		// String pathdeptId =
		// DepartmentCache.getModel(CmccCommon.deptid_jsb).getPathIdOfCache();//ces
		String pathdeptId = DepartmentCache.getModel("5b93a3f7-3ae2-4831-9fcc-d8809ffc462c").getPathIdOfCache();// zs
		// 获取需求部门企标管理员所在部门ID全路径的集合
		if (UtilString.isEmpty(sql_demandPeople)) {
			//////////////////
			// if(pathdeptId.equals("ec76faca-0265-4666-bd72-567934551f6f")){
			sql_demandPeople = "SELECT GROUP_CONCAT(userid) userid " + "FROM BO_ACT_ENTERPRISE_ROLEDATA "
					+ "WHERE ROLENAME = '预算接口人' AND locate(deptid, '" + pathdeptId + "' ) > 0";
			/*
			 * }else{ sql_demandPeople = "SELECT GROUP_CONCAT(userid) userid " +
			 * "FROM BO_ACT_ENTERPRISE_ROLEDATA " +
			 * "WHERE ROLENAME = '集团企标管理员' AND locate(deptid, '"
			 * +pathdeptIds+"' ) > 0"; }
			 */
		...
		/*
		 * 根据父任务状态查找人员范围 如果为1，则查找需求部所企标管理员 如果为企标管理员则查找所有人， 如果不是企标管理员只查找本部所所有人
		 */
		if (taskstate == 1) {
			if (list.size() > 0 && list.toString().contains(deptid_query)) {
				return true;
			}
		} else {
			...
			if (!UtilString.isEmpty(count) && Integer.parseInt(count) > 0) {// wxx
				...
				if (userid.contains("@hq.cmcc")) {
					...
					// 查询本单位所有部所
					if (deptid_query.contains(parentDeptId_current) || pathParentDeptid.contains(deptid_query)) {
						return true;
					}
				} else {
					// 一级部门ID
	...
	public boolean addressUIFlexUserFilter(UserContext uc, UserModel model, AdvancedAddressModel advancedAddressModel) {
		...
		/*
		 * if (UtilString.isEmpty(deptId)) { deptId =
		 * DBSql.getString(sql_deptId, new Object[] { processId }); }
		 */
		/*
		 * if("".equals(deptId)){ return false; }
		 */
		// 技术部门ID全路径即为技术部门ID全路径
		// String pathdeptId =
		// DepartmentCache.getModel(CmccCommon.deptid_jsb).getPathIdOfCache();
		String pathdeptId = DepartmentCache.getModel("5b93a3f7-3ae2-4831-9fcc-d8809ffc462c").getPathIdOfCache();
		// 获取需求部门企标管理员所在部门ID全路径的集合
		if (UtilString.isEmpty(sql_demandPeople)) {
			sql_demandPeople = "SELECT GROUP_CONCAT(userid) userid " + "FROM BO_ACT_ENTERPRISE_ROLEDATA "
					+ "WHERE ROLENAME = '预算接口人' AND locate(deptid, '" + pathdeptId + "' ) > 0";
		...
		/*
		 * 根据父任务状态查找人员范围 如果为1，则查找需求部所企标管理员 否则，查找需求单位所有人
		 */
		String userid = uc.getUID();
		if (taskstate == 1) {
			if (userid_list.size() > 0 && userid_list.toString().contains(model.getUID())) {
				return true;
			}
			return false;
		} else {
