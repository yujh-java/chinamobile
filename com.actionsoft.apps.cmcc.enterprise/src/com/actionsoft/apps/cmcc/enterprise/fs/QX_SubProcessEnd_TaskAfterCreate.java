 ...
 * @date 20190417
 */
public class QX_SubProcessEnd_TaskAfterCreate extends ExecuteListener implements ExecuteListenerInterface{
	...
	public void execute(ProcessExecutionContext param) throws Exception {
		...
		if (task_acticityId.equals(QbActivityIdConst.ysbmjkr)) {// 子流程最后一个节点
			acticityId = QbActivityIdConst.jsbxqbmhq;// 主流程中技术部和需求部门会签节点
		} 
		...
		try{
			...
			List<BO> list = SDK.getBOAPI().query("BO_ACT_CMCC_PROCESSHANDLE").addQuery("PROCESSID = ", parentProcessId)
					.addQuery("ACTIVITYID = ", "obj_c868861a06c0000131ad16d01e541f21").connection(conn).list();
