...
public class FilterlistutilSystem {
	/**
	 * 过滤历史任务
	 * @param list
	 * @return
	 */
	public static List<Map<String,Object>> filterHisList(List<HistoryTaskInstance> list,List<TaskInstance> listJqlHisTask,String system,String processStates){
		...
		if(list != null && list.size() > 0){
			for(int i = 0;i<list.size();i++){
				...
				boolean isEnd = SDK.getProcessAPI().isEndById(processId);
				...
				if(map != null && map.size() > 0){
					if(!map.containsKey(processId+target)){
						...
						if(!CMCCConst.NotDoHistoryTask.toString().contains(ACTIVITYDEFID)){
							...
							String type="";
							List<BO> list_processData = SDK.getBOAPI().query("BO_ACT_PROCESS_DATA").addQuery("PROCESSDEFNID = ", processDefid).list();
							if(list_processData != null && list_processData.size() > 0){
//								type = list_processData.get(0).getString("PROCESSTYPE");
								//chenxf modify 20180808
								type = list_processData.get(0).getString("SYSTEM");
							}
							...
							if(type.contains(system)&&!UtilString.isEmpty(system)&&!UtilString.isEmpty(processStates)){
								if(processStates.equals("all")){
									newlist.add(filter);
								}else if(processStates.equals("end")&&isEnd){
									newlist.add(filter);
								}else if(processStates.equals("pending")&&!isEnd){
									newlist.add(filter);
								}
			...
			for(int i = 0;i<listJqlHisTask.size();i++){
				...
				if(map != null && map.size() > 0){
					if(!map.containsKey(processId+target)){
						...
						if(!CMCCConst.NotDoHistoryTask.toString().contains(ACTIVITYDEFID)){
							String processDefid =listJqlHisTask.get(i).getProcessDefId();
							String type="";
							List<BO> list_processData = SDK.getBOAPI().query("BO_ACT_PROCESS_DATA").addQuery("PROCESSDEFNID = ", processDefid).list();
							if(list_processData != null && list_processData.size() > 0){
//								type = list_processData.get(0).getString("PROCESSTYPE");
