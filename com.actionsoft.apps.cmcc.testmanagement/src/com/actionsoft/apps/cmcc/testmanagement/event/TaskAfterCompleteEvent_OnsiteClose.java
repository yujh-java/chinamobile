...
* 任务生成后，回写状态（现场测试流程专用：close起草,flag=2为任务通过）
*/
public class TaskAfterCompleteEvent_OnsiteClose extends ExecuteListener implements ExecuteListenerInterface{
	public String getDescription() {
		return "任务生成后，回写状态（现场测试流程专用：close起草,flag=2为任务通过）";
	}
	@Override
		...
		if(taskState == 1 || taskState == 4 || taskState==11){
			...
			try{
				...
				if(!UtilString.isEmpty(dateStr)){
					...
					sb.append("&dayTime=");
					sb.append(dayTime);
					if(processDefId.equals(TestManagementConst.CSJL_ID)){
						sb.append("&flag=1");
					}else if(processDefId.equals(TestManagementConst.WDJL_ID)){
						sb.append("&flag=3");
					}
					sb.append("&flag=2");
