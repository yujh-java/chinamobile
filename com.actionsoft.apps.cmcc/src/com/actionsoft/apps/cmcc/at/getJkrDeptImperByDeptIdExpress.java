...
public class getJkrDeptImperByDeptIdExpress extends AbstExpression {

	public getJkrDeptImperByDeptIdExpress(ExpressionContext atContext,
	...
	public String execute(String expression) throws AWSExpressionException {
		...
		String preNoteid = getParameter(expression, 5);//节点id
		String noteid = getParameter(expression, 6);//当前节点id
		
		if(!UtilString.isEmpty(preNoteid)&&!UtilString.isEmpty(noteid)&&preNoteid.equals(noteid)){
			List <HistoryTaskInstance> hlist = SDK.getHistoryTaskQueryAPI().activityDefId(activityId).processInstId(processId).taskState(1).orderByEndTime().asc().list();
			if(hlist!=null&&hlist.size()>0){
				String target = hlist.get(hlist.size()-1).getTarget();
				return target;
			}
		}
