...
public class Sign_LeaderUsersAddress2 implements AddressUIFilterInterface {
	...
	public boolean addressUIFlexDepartmentFilter(UserContext uc, DepartmentModel model, AdvancedAddressModel admodel) {
		...
		boolean	 bol = SDK.getTaskAPI().isChoiceActionMenu(taskid,"送研发机构项目承担部所内部会签");
		...
		if(!bol){
			...
			if(deptPathId.contains(userPathId)||userPathId.contains(deptPathId)){
				return true;
			}
