		...
		if(taskState == 1 || taskState == 4 || taskState == 11){
			...
			try{
				...
				if(!UtilString.isEmpty(dateStr)){
					...
					String title = param.getProcessInstance().getTitle();//标题
					String submitterid = param.getProcessInstance().getCreateUser();//流程创建者
					String processType = DBSql.getString(conn,"SELECT PROCESSTYPE FROM BO_ACT_PROCESS_DATA WHERE PROCESSDEFNID = ?", new Object[]{process_defid});//流程类型
