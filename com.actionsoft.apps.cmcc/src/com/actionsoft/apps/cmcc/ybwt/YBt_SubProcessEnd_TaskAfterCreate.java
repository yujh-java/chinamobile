 ...
 * @date 20190617
 */
public class YBt_SubProcessEnd_TaskAfterCreate extends ExecuteListener implements ExecuteListenerInterface{
	...
	public void execute(ProcessExecutionContext param) throws Exception {
		...
		String acticityId = "";
		if (task_acticityId.equals(YBActivityIdConst.ysbmjkr)) {// 子流程最后一个节点
			acticityId = YBActivityIdConst.qtyfjkr;// 主流程中技术部企标管理员处理节点
		} 
		...
		try{
			...
			List<BO> list = SDK.getBOAPI().query("BO_ACT_CMCC_PROCESSHANDLE").addQuery("PROCESSID = ", parentProcessId)
					.addQuery("ACTIVITYID = ", "obj_c8789df0ece0000174381d3d57907380").connection(conn).list();
