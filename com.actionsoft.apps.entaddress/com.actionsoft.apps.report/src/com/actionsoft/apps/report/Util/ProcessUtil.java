...
public class ProcessUtil {
	...
	public static String getNoteName(String processid, String activityId, String processDefid) {
		...
		try {
			conn = DBSql.open();
			if(UtilString.isEmpty(activityId)){
			List<HistoryTaskInstance> his =	SDK.getHistoryTaskQueryAPI().processInstId(processid).orderByEndTime().asc().list();
			notename = ProcessUtil.getNoteIdName(
					his.get(his.size() - 1).getProcessDefId(),
					his.get(his.size() - 1).getActivityDefId());
				System.err.println("noteName="+notename);
			}else{
		...
		if (ctask != null && ctask.size() > 0) {
						...
						if (!UtilString.isEmpty(hislist)) {
							...
							if (activityType.equals("callActivity")) {
								List<RowMap> htask = DBSql.getMaps(
										"SELECT * FROM WFH_TASK  WHERE PROCESSINSTID IN (SELECT ID from WFC_PROCESS where parentprocessinstid=?) ORDER BY ENDTIME ASC",
			...
			List<ProcessInstance> subList = SDK.getProcessQueryAPI().parentProcessInstId(processid).activeProcess()
					.list();
			if (subList != null && subList.size() > 0) {
				for (int i = 0; i < subList.size(); i++) {
					...
					if (subtask != null && subtask.size() > 0) {
						/*
						 * List<HistoryTaskInstance> histask =
						 * SDK.getHistoryTaskQueryAPI().processInstId(id)
						 * .orderByEndTime().connection(conn).list();
						 */
						for (int j = 0; j < subtask.size(); j++) {
							...
							if (!UtilString.isEmpty(histask)) {
								...
								List<HistoryTaskInstance> hitask = SDK.getHistoryTaskQueryAPI().processInstId(processid)
	...
	public static String getCostTime(String createTime,String processEndTime) throws ParseException{
		...
		if (!UtilString.isEmpty(processEndTime)) {
			...
			sbf.append(day).append(" ").append(df.format(hour)).append(":").append(df.format(minute)).append(":").append(df.format(second));
		}
		return sbf.toString();
	}
		...
		if (taskinstid.equals("00000000-0000-0000-0000-000000000000")) {
			...
			if (!UtilString.isEmpty(pinstace)) {
				...
				if (UtilString.isEmpty(parentProid)) {
					...
					List<HistoryTaskInstance> histask = SDK.getHistoryTaskQueryAPI().processInstId(parentProid)
							.orderByEndTime().asc().connection(conn).list();
					if (histask != null && histask.size() > 0) {
						noteName = ProcessUtil.getNoteIdName(histask.get(histask.size() - 1).getProcessDefId(),
								histask.get(histask.size() - 1).getActivityDefId());
					}
				}
					...
					if (!UtilString.isEmpty(his)) {
						...
						if (taskstate == 11) {
							...
							HistoryTaskInstance his2 = SDK.getHistoryTaskQueryAPI().connection(conn)
									.detailById(parentTaskid);
							if (!UtilString.isEmpty(his2)) {
								noteName = ProcessUtil.getNoteIdName(his2.getProcessDefId(), his.getActivityDefId());
							}
						}
				...
				if (!UtilString.isEmpty(his)) {
					if ("callActivity".equals(his.getActivityType())) {
						...
						if (task != null && task.size() > 0) {
							noteName = ProcessUtil.getNoteIdName(task.get(task.size() - 1).getString("PROCESSDEFID"),
